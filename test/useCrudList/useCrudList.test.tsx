...
describe('useCrudList', () => {
    ...
    it('should add items on create', async () => {
        ...
        expect(hook.result.current.list).toEqual([...defaultItems, { id: 3, name: 'Charlie' }]);
    });
    ...
    it('should change items on update', async () => {
        ...
        expect(hook.result.current.list).toEqual([defaultItems[0], { id: 2, name: 'Charlie' }]);
    });
    ...
    it('should recreate item on custom hook', async () => {
        ...
        await hook.waitFor(() => hook.result.current.recreate.mutation.isSuccess);
        expect(hook.result.current.list).toEqual([defaultItems[1], { id: 3, name: 'Alice' }]);
    });
    ...
    it('should clear items on custom hook', async () => {
        ...
        await hook.waitFor(() => hook.result.current.clear.mutation.isSuccess);
        expect(hook.result.current.list).toEqual([]);
    });
});
